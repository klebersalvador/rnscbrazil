...
export default function GerenciarProvas() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Buscar Evento
      const resEvento = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/eventos/${id}`, { headers });
      const dataEvento = await resEvento.json();
      setEvento(dataEvento);

      // Buscar Provas do evento
      const resProvas = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/provas?id_evento=${id}`, { headers });
      const dataProvas = await resProvas.json();
      setProvas(dataProvas);

      // Buscar Divisões disponíveis
      const resDivisoes = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/divisoes?limit=1000`, { headers });
  ...
  const handleEditProva = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/provas/${provaEditando.id_prova}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSaveFasesConfig = async () => {
    ...
    try {
      const token = localStorage.getItem('rsnc_token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/provas/${provaConfigFases.id_prova}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('rsnc_token');
      const res = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/provas/${itemToDelete}`, {
        method: 'DELETE',
